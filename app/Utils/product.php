...
if (!function_exists('getPriceRangeWithDiscount')) {
    ...
    {
        /**
         * PSF — no price means "prix sur demande".
         * Guarded here so every template that prints a price range
         * (sticky bar, cards, ranges) shows the right text.
         */
        if ($type === 'web' && (float)($product->unit_price ?? 0) <= 0) {
            return '<span class="discounted-unit-price psf-price-on-request fs-20 fw-bold lh-1">'
                . e((string)(translate('Contact_us_for_the_price') ?: 'Contactez-nous pour le prix'))
                . '</span>';
        }
