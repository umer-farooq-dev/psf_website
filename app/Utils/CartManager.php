...
class CartManager
{
    ...
    public static function add_to_cart($request, $from_api = false): array
    {
        ...
        /**
         * PSF — a product with no price is "prix sur demande": it is ordered
         * through WhatsApp, never through the cart. Guarded here because every
         * add-to-cart path (web, API, wishlist bulk add) goes through this method,
         * so a crafted request cannot slip past the UI.
         */
        if (!psfHasPrice($product)) {
            return ['status' => 0, 'message' => translate('Contact_us_for_the_price')];
        }
