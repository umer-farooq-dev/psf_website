...
if (!function_exists('getProductPriceByType')) {
    ...
    {
        /**
         * PSF — products with no price are "prix sur demande".
         * Intercepting here means every storefront template that prints a
         * price gets the right text, without touching each one.
         *
         * Only the displayed string on the website is replaced:
         *  - $result === 'value' still returns a number (calculations),
         *  - $from === 'panel' still shows the raw figure to the admin,
         *  - a variant price passed in $price is respected.
         */
        if ($type === 'discounted_unit_price' && $result !== 'value' && $from === 'web'
            && (float)$price === 0.0 && (float)($product['unit_price'] ?? 0) <= 0) {
            // translate() can return null; this function must return string|int|float
            return (string)(translate('Contact_us_for_the_price') ?: 'Contactez-nous pour le prix');
        }
