...
/**
 * ---------------------------------------------------------------
 * 7. BRAND — accent colour, slogan, Google services (brief §2/§20/§23)
 *    6valley only ships primary + secondary; PSF's identity is
 *    dark blue + cyan + yellow + white, with yellow reserved for the
 *    important calls-to-action.
 * ---------------------------------------------------------------
 */

if (!function_exists('psfAccentColor')) {
    /**
     * Third brand colour, used for the main calls-to-action.
     *
     * business_settings -> `psf_accent_color`. Any value that is not a
     * plain hex code is ignored, so a bad paste can never inject CSS.
     */
    function psfAccentColor(): string
    {
        static $cached = null;
        if ($cached !== null) {
            return $cached;
        ...
        $value = trim((string)(getWebConfig(name: 'psf_accent_color') ?: ''));

        return $cached = preg_match('/^#[0-9a-fA-F]{3,8}$/', $value) ? $value : '#f5c518';
    }
}

if (!function_exists('psfAccentTextColor')) {
    /**
     * Readable text colour on top of the accent, chosen by luminance so a
     * dark accent gets white text and a light one gets near-black.
     */
    function psfAccentTextColor(): string
    {
        $hex = ltrim(psfAccentColor(), '#');
        if (strlen($hex) === 3) {
        ...
        // Rec. 601 luma — good enough to pick between two text colours.
        return (0.299 * $r + 0.587 * $g + 0.114 * $b) > 150 ? '#12202e' : '#ffffff';
    }
}

if (!function_exists('psfSlogan')) {
    /**
     * Company slogan (brief §2 / §23). Empty means the block is not shown.
     */
...
/* Google Analytics is 6valley's own feature — Third Party -> Analytics
   (google_analytics, google_tag_manager, meta_pixel and others). Nothing is
   added here for it; the client only has to paste their ID there. */

if (!function_exists('psfGoogleBusinessUrl')) {
    /**
     * Link to the Google Business Profile (brief §20). https only.
     */
    function psfGoogleBusinessUrl(): string
    {
        $value = trim((string)(getWebConfig(name: 'psf_google_business_url') ?: ''));

        return str_starts_with(strtolower($value), 'https://') ? $value : '';
    }
}

if (!function_exists('psfAboutMenu')) {
    /**
     * Whether "À propos" is shown in the site menus (brief §4).
     * business_settings -> `psf_about_menu`. Defaults to on.
     */
    function psfAboutMenu(): bool
    {
        static $cached = null;
        if ($cached === null) {
            $value = getWebConfig(name: 'psf_about_menu');
            $cached = $value === null || $value === '' || (bool)$value;
        }
...
if (!function_exists('psfOpeningHoursSummary')) {
    /**
     * Opening hours as one short line, for the footer.
     *
     * Consecutive days sharing the same hours are folded into a range, so a
     * normal week reads "Lundi – Samedi : 07:30 - 18:00 · Dimanche : Fermé"
     * instead of seven separate rows. Returns '' when no day has hours set,
     * so the footer shows nothing rather than a column of "Fermé".
     */
    function psfOpeningHoursSummary(): string
    {
        $days = psfOpeningHours();
        if ($days === []) {
            return '';
        }
        ...
        $groups = [];
        foreach ($days as $day) {
            $value = ($day['closed'] || $day['hours'] === '') ? translate('Closed') : $day['hours'];
            $last = count($groups) - 1;
            if ($last >= 0 && $groups[$last]['value'] === $value) {
                $groups[$last]['to'] = $day['day'];
            } else {
                $groups[] = ['from' => $day['day'], 'to' => $day['day'], 'value' => $value];
            }
        }

        $parts = [];
        foreach ($groups as $group) {
            $label = $group['from'] === $group['to'] ? $group['from'] : $group['from'] . ' – ' . $group['to'];
...
if (!function_exists('psfWhatsappOrderData')) {
    /**
     * Everything the product page needs to build an order message in the
     * browser (client brief §8).
     *
     * `{quantity}` and `{variation}` are deliberately left unreplaced: only
     * the browser knows what the visitor picked, so the button fills them in
     * on click. Any line still holding an empty placeholder is dropped, so a
     * product without variations never sends "Taille :" with nothing after it.
     *
     * @param object|array|null $product
     * @return array{number:string,template:string}
     */
    function psfWhatsappOrderData($product): array
    {
        $template = psfWhatsappTemplate(
            'psf_order_request_template',
            'Bonjour PSF, je souhaite commander ce produit : {product}'
                . PHP_EOL . 'Quantité : {quantity}'
                . PHP_EOL . '{variation}'
                . PHP_EOL . '{url}'
        );

        return [
            'number'   => psfWhatsappNumber(),
            'template' => psfFillTemplate($template, $product),
        ];
    }
}
