...
let reviewTicketSelectedFiles = [];
$(document).on('ready', () => {
    $(".attachmentfiles").on('change', function () {
        for (let i = 0; i < this.files.length; ++i) {
            reviewTicketSelectedFiles.push(this.files[i]);
        }
    ...
    function displaySelectedFiles(pre_container = null) {
        ...
        container.innerHTML = "";
        reviewTicketSelectedFiles.forEach((file, index) => {
        ...
        for (let i = 0; i < reviewTicketSelectedFiles.length; ++i) {
            ...
            filereader.readAsDataURL(reviewTicketSelectedFiles[i]);
            ...
            $(uploadDiv).find('.img-clear').on('click', function () {
                ...
                reviewTicketSelectedFiles.splice(i, 1);
