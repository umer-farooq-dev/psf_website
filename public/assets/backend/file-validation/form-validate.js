(function (global, $) {
    ...
    function showErrorToast(message, queued = false) {
        // When in queue mode (submit), push message instead of replacing
        if (queued) {
        ...
        // Real-time single toast (default behavior)
    ...
    function processToastQueue() {
        ...
        // Show toast (without clearing old ones)
        if (typeof toastMagic !== "undefined") {
        ...
        // delay
        setTimeout(processToastQueue, 1200);
    }
    ...
    function initValidationForForm($form) {
        ...
        $form.validate({
            ...
            invalidHandler: function (e, validator) {
                ...
                scrollToErrorField(errors[0].element);

                // Queue all error toasts
                errors.forEach((err) => {
                    const $el = $(err.element);
                    let msg = err.message;
                    ...
                    if (err.method === "required") {
                        msg = $el.data('required-msg') ||
                            $("#text-validate-translate").data("required-field") ||
                            msg;
                    }
                ...
                const totalDelay = toastQueue.length * 1200;
                setTimeout(() => {
                    $submitBtn.prop('disabled', false);
                }, totalDelay + 500);
            },
    ...
    $(function () {
        // ------ INIT EXISTING FORMS
         ...
         $(document).on("submit", ".custom-validation", function(e) {
            ...
            // If valid, allow the form to continue, whether it's AJAX or normal submit
        });
