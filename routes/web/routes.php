...
Route::group(['namespace' => 'Web', 'middleware' => ['maintenance_mode', 'guestCheck']], function () {
    ...
    Route::controller(\App\Http\Controllers\Web\PsfQuoteRequestController::class)
        ->prefix('demande-de-devis')->name('psf.quote.')->group(function () {
            Route::get('/', 'index')->name('index');
            Route::post('/', 'store')->name('store');
        });

    // PSF — Nos réalisations (client brief §17)
    Route::controller(\App\Http\Controllers\Web\PsfGalleryController::class)
        ->prefix('nos-realisations')->name('psf.gallery.')->group(function () {
            Route::get('/', 'index')->name('index');
        });
