<div>
    <form class="order-edit-add-to-cart-form" method="POST"
          action="{{ route('admin.orders.edit-order-product-add') }}"
          data-check-variant-price="{{ route('admin.orders.edit-order-product-variant-price') }}">
        @csrf
            ...
            <div class="details">
                <div class="d-flex flex-wrap gap-3 align-items-center mb-4">
                    @if($product['product_type'] === 'physical')
                        ...
                        <div
                            class="d-flex gap-2 align-items-center rounded-pill {{ $currentVariation['current_stock'] > 0 ? 'bg-success text-success' : 'bg-danger text-danger' }} bg-opacity-10 px-2 py-1">
                    ...
                    @if (getProductPriceByType(product: $product, type: 'discount', result: 'value') > 0)
                        <div
                            class="d-flex gap-1 align-items-center rounded-pill px-2 py-1 text-primary bg-primary bg-opacity-10">
                            <span class="fs-12">
                ...
                @if ($product['reviews_count'] > 0)
                    <div class="d-flex align-items-center gap-2 mb-3">
                        ...
                        <span class="text-muted text-capitalize">
                            ({{ $product['reviews_count'] . ' ' . translate('customer_review') }})
                        </span>
                    </div>
                @endif
            ...
            <div class="variant-change">
                ...
                @if (
                    ...
                    @foreach ($product['digital_product_extensions'] as $extensionKey => $extensionGroup)
                        <div class="d-flex gap-3 align-items-center mb-3">
                            <div class="my-2">
                                <span class="text-9B9B9B flex-shrink-0">
                                ...
                                <div class="d-flex gap-2 flex-wrap">
                                    @foreach ($extensionGroup as $index => $extension)
                                        ...
                                        <label
                                            ...
                                            <span class="text-nowrap max-w-180 line-1">
                                                {{ $extension }}
                                            </span>
                                        </label>
                                        @php($extensionIndex++)
        ...
        <div class="d-flex flex-wrap align-items-center gap-3 position-relative price-section mt-1">
            <div
                class="alert alert--message flex-row alert-dismissible fade show pos-alert-message pos-bg-warning fs-12 px-12 py-10 text-dark rounded d-flex gap-2 align-items-center justify-content-between d-none"
                role="alert">
                <div class="d-flex gap-2 align-items-center">
                    <img class="mb-1"
        ...
        <div class="d-flex justify-content-center mt-4">
            @if($product['product_type'] == 'physical')
                <button class="btn btn-primary btn-block" type="submit" data-loading-text="{{ translate('Checking Price') }}">
