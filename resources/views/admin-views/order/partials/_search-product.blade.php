@if (count($products) > 0)
    @foreach ($products as $key => $product)
        <div 
            class="select-order-edit-product-item @if($product->product_type == 'digital') disabled @endif" 
            data-product-id="{{ $product['id'] }}" data-order-id="{{ $orderId }}">
            <div>
                <div
                    class="media gap-3 p-3 rounded cursor-pointer justify-content-between align-items-center flex-wrap flex-xl-nowrap"
                    data-id="1">
                    <div class="d-flex align-items-center gap-3">
                        <img class="avatar avatar-xl border rounded border object-fit-cover" width="75"
                        ...
                        <div class="media-body d-flex flex-column gap-1">
                            ...
                            <div class="d-flex gap-2 align-items-center mb-1">
                                <h4 class="mb-0 product-name line-1 text-start">
                                    {{ $product['name'] }}
                                </h4>
                                @if($product->product_type == 'digital')
                                    <span class="flex-shrink-0 fs-12 text-danger bg-danger bg-opacity-10 fw-medium px-2 py-1 rounded-pill">{{ translate('Unable_to_add') }}</span>
                                @endif
                            </div>
