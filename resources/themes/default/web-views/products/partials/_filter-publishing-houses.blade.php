            ...
            @foreach($productPublishingHouses as $publishingHouseItem)
                <?php
                    if (isset($dataFrom) && $dataFrom == 'shop-view' && isset($shopSlug)) {
                        $publishingHouseRoute = route('vendor-shop', ['slug' => $shopSlug, 'publishing_house_id' => $publishingHouseItem['id'], 'product_type'=> 'digital', 'offer_type' => ($data['offer_type'] ?? ''), 'page' => 1]);
                        ...
                        $params = ['publishing_house_id' => $publishingHouseItem['id'], 'product_type' => 'digital', 'offer_type' => ($data['offer_type'] ?? ''), 'page' => 1];
                        $publishingHouseRoute = route('products', $params);
                    }
                ?>
