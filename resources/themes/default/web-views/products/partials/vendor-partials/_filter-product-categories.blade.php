                ...
                <div class="card-header flex-between">
                    <?php
                        if (isset($dataFrom) && $dataFrom == 'shop-view' && isset($shopSlug)) {
                            $categoryRoute = route('vendor-shop', ['slug' => $shopSlug, 'category_id' => $category['id'],'data_from'=>'category', 'offer_type' => ($data['offer_type'] ?? ''), 'page' => 1]);
                            ...
                            $categoryRoute = route('vendors', ['category_id' => $category['id'],'data_from'=>'category', 'offer_type' => ($data['offer_type'] ?? ''), 'page' => 1]);
                        }
                    ?>
                    <div>
                    ...
                    @foreach($category->childes as $child)
                        ...
                        <div class="menu--caret-accordion {{ $dropdownActive ? 'open' : '' }}">
                            <div class="for-hover-label card-header flex-between">
                                <?php
                                    $subCategoryRoute = route('vendors', ['sub_category_id' => $child['id'],'data_from'=>'category', 'offer_type' => ($data['offer_type'] ?? ''), 'page' => 1]);
                                ?>
                            ...
                            <div
                                ...
                                @foreach($child->childes as $subSubCategory)
                                    <?php
                                        $subSubCategoryRoute = route('vendors', ['sub_sub_category_id' => $subSubCategory['id'], 'data_from' => 'category', 'offer_type' => ($data['offer_type'] ?? ''), 'page' => 1]);
                                    ?>
