            ...
            @foreach($productAuthors as $productAuthor)
                <?php
                    if (isset($dataFrom) && $dataFrom == 'shop-view' && isset($shopSlug)) {
                        $productAuthorRoute = route('vendor-shop', ['slug' => $shopSlug, 'author_id' => $productAuthor['id'], 'product_type'=> 'digital', 'offer_type' => ($data['offer_type'] ?? ''), 'page' => 1]);
                        ...
                        $params = ['author_id' => $productAuthor['id'], 'product_type' => 'digital', 'offer_type' => ($data['offer_type'] ?? ''), 'page' => 1];
                        $productAuthorRoute = route('products', $params);
