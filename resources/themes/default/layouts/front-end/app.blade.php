...
<head>
    ...
    {{-- PSF: the robots tag is emitted once, by the SEO partial below, using
         the page's own panel record or the site-wide default. It used to be
         hardcoded here as "noindex, nofollow", which kept the whole site out
         of Google. --}}
    ...
    {{-- PSF: structured data for search engines (client brief §20) --}}
    ...
    <style>
        :root {
            ...
            /* PSF: third brand colour, reserved for the main calls-to-action
               (client brief §2 / §24). Set from Paramètres PSF. */
        ...
        /* PSF: the accent button, used only for the most important calls to
           action (client brief §24 — "yellow for important calls-to-action").
           Colours come from the variables above, so they follow the panel. */
        .psf-accent-btn,
        .psf-accent-btn:hover,
        .psf-accent-btn:focus,
        .psf-accent-btn:active {
            background-color: var(--psf-accent);
            border-color: var(--psf-accent);
            color: var(--psf-accent-text);
        }
    ...
    <div class="__floating-btn">
        ...
        @if(isset($whatsapp['status']) && $whatsapp['status'] == 1 )
            <div class="wa-widget-send-button">
                {{-- PSF: greeting comes from the panel, not from code --}}
                <a href="{{ psfWhatsappChatUrl() }}" target="_blank" rel="noopener">
