...
Route::group(['prefix' => 'admin', 'as' => 'admin.', 'middleware' => ['admin', 'actch:admin_panel']], function () {
    ...
    Route::group(['prefix' => 'demandes-de-devis', 'as' => 'psf-quote.'], function () {
        Route::controller(\App\Http\Controllers\Admin\PsfQuoteRequestController::class)->group(function () {
            Route::get('/', 'index')->name('index');
    ...
    Route::group(['prefix' => 'realisations', 'as' => 'psf-gallery.'], function () {
        Route::controller(\App\Http\Controllers\Admin\PsfGalleryController::class)->group(function () {
            Route::get('/', 'index')->name('index');
            Route::get('ajouter', 'create')->name('create');
            Route::post('ajouter', 'store')->name('store');
            Route::get('modifier/{id}', 'edit')->name('edit');
            Route::post('modifier/{id}', 'update')->name('update');
            Route::post('statut/{id}', 'updateStatus')->name('status');
            Route::post('supprimer/{id}', 'delete')->name('delete');
        });
    });

    // PSF — every PSF-specific option, so nothing lives in the code
    Route::group(['prefix' => 'parametres-psf', 'as' => 'psf-settings.'], function () {
