            ...
            <div class="container text-center __pb-13px">
                ...
                <div
                    ...
                    <div class="col-md-3 footer-web-logo text-center text-md-start ">
                        ...
                        {{-- PSF: company slogan (client brief §2 / §23) — set in Paramètres PSF --}}
                        @if (psfSlogan())
                            <p class="__text-14px mt-2 mb-0 fst-italic">{{ psfSlogan() }}</p>
                        @endif
                            ...
                            <div class="col-sm-3 col-6 footer-padding-bottom text-start">
                                ...
                                <ul class="widget-list __pb-10px">
                                    ...
                                    @if (psfGalleryMenu())
                                        <li class="widget-list-item">
                                            <a class="widget-list-link"
                                               href="{{ route('psf.gallery.index') }}">{{ translate('Our_Realisations') }}</a>
                                        </li>
                                    @endif
                                    @if (psfQuoteMenu())
                                        <li class="widget-list-item">
                                            <a class="widget-list-link"
                                               href="{{ route('psf.quote.index') }}">{{ translate('Quote_Request') }}</a>
                                        </li>
                                    @endif

                                    {{-- PSF: product categories in the footer (client brief §23).
                                         Reads the live category list — nothing to maintain by hand. --}}
                                    @php($psfFooterCategories = \App\Models\Category::where('position', 0)
                                        ->orderBy('priority')->orderBy('name')->take(8)->get())
                                    @if ($psfFooterCategories->count() > 0)
                                        <li class="widget-list-item mt-3">
                                            <span class="footer-header font-semi-bold text-uppercase mobile-fs-12">
                                                {{ translate('categories') }}
                                            </span>
                                        </li>
                                        @foreach ($psfFooterCategories as $psfFooterCategory)
                                            <li class="widget-list-item">
                                                <a class="widget-list-link"
                                                   href="{{ route('products', ['category_id' => $psfFooterCategory->id]) }}">
                                                    {{ $psfFooterCategory->name }}
                                                </a>
                                            </li>
                                        @endforeach
                                    @endif
                ...
                <div class="row g-4 {{Session::get('direction') === "rtl" ? ' flex-row-reverse' : ''}}">
                    ...
                    <div class="col-xl-4 col-md-6 text-start">
                        ...
                        {{-- PSF: opening hours, directions and Google profile (client brief §23).
                             All read from Paramètres PSF — the same values as the contact page. --}}
                        @php($psfFooterHours = psfOpeningHours())
                        @if (count($psfFooterHours) > 0)
                            <div class="mt-2">
                                <span class="__text-14px d-flex align-items-start">
                                    <i class="fa fa-clock-o me-2 mt-2 mb-2"></i>
                                    <span>
                                        @foreach ($psfFooterHours as $psfFooterDay)
                                            <span class="d-block">
                                                {{ $psfFooterDay['day'] }} :
                                                @if ($psfFooterDay['closed'] || $psfFooterDay['hours'] === '')
                                                    {{ translate('Closed') }}
                                                @else
                                                    {{ $psfFooterDay['hours'] }}
                                                @endif
                        ...
                        <div class="mt-2 d-flex flex-wrap gap-3">
                            @if (psfDirectionsUrl())
                                <a class="widget-list-link __text-14px" target="_blank" rel="noopener"
                                   href="{{ psfDirectionsUrl() }}">
                                    <i class="fa fa-location-arrow me-1"></i> {{ translate('Get_Directions') }}
                                </a>
                            @endif
                            @if (psfGoogleBusinessUrl())
                                <a class="widget-list-link __text-14px" target="_blank" rel="noopener"
                                   href="{{ psfGoogleBusinessUrl() }}">
                                    <i class="fa fa-google me-1"></i> {{ translate('Our_Google_Page') }}
                                </a>
                            @endif
                        </div>
                    </div>
