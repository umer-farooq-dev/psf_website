    ...
    <div class="aside-body search-aside-attribute-container py-4 pt-0">
        ...
        <ul class="aside-nav navbar-nav gap-2">
            ...
            @if(Helpers::module_permission_check('support_section'))
                ...
                {{-- PSF: Demandes de devis (client brief §16) --}}
                @if (psfMenu('quote_requests'))
                    @php($psfNewQuotes = \App\Models\PsfQuoteRequest::where('seen', false)->count())
                    <li>
                        <a class="nav-link {{ Request::is('admin/demandes-de-devis*') ? 'active' : '' }}"
                           title="{{ translate('Quote_Requests') }}"
                           href="{{ route('admin.psf-quote.index') }}">
                            <i class="fi fi-sr-document-signed"></i>
                            <span class="aside-mini-hidden-element text-truncate">
                                {{ translate('Quote_Requests') }}
                            </span>
                            @if ($psfNewQuotes > 0)
                                <span class="badge badge-soft-danger ms-auto aside-mini-hidden-element">
                                    {{ $psfNewQuotes }}
                                </span>
                            @endif
                        </a>
                ...
                {{-- PSF: Nos réalisations (client brief §17) --}}
                @if (psfMenu('gallery'))
                    <li>
                        <a class="nav-link {{ Request::is('admin/realisations*') ? 'active' : '' }}"
                           title="{{ translate('Our_Realisations') }}"
                           href="{{ route('admin.psf-gallery.index') }}">
                ...
                {{-- PSF: every PSF option in one place --}}
                <li>
                    <a class="nav-link {{ Request::is('admin/parametres-psf*') ? 'active' : '' }}"
                       title="{{ translate('PSF_Settings') }}"
                       href="{{ route('admin.psf-settings.index') }}">
            ...
            @if(Helpers::module_permission_check('report'))
                ...
                @if(getCheckAddonPublishedStatus(moduleName: 'TaxModule'))
                    @foreach(include(base_path("Modules/TaxModule/Addon/tax_report_routes.php")) as $route)
                        {{-- PSF: this loop also renders the admin tax report, so only the
                             vendor one is skipped — panel-managed, the module is untouched. --}}
                        @if($route['name'] === 'Vendor_Vat_Report' && !psfMenu('vendor_vat_report'))
                            @continue
                        @endif
                        <li>
                            <a class="nav-link
            ...
            @if (psfMenu('blog')) {{-- PSF: panel-managed --}}
            ...
            @if (psfMenu('employees')) {{-- PSF: panel-managed --}}
...
{{-- PSF: the floating "Setup Guide" card, panel-managed via
     Paramètres PSF → Visibilité des menus (key: setup_guide). --}}
@if (psfMenu('setup_guide'))
