                                ...
                                <div class="col-lg-7 col-md-8 mt-md-0 mt-sm-3 web-direction">
                                    <div class="details __h-100 product-cart-option-container p-0">
                                        ...
                                        <form class="addToCartDynamicForm add-to-cart-details-form d-flex flex-column gap-4">
                                            ...
                                            <div class="">
                                                {{-- PSF: price shown only when one was entered --}}
                                                @if (psfHasPrice($product))
                                                ...
                                                @else
                                                    <h3 class="font-weight-normal text-accent fs-20 font-bold mb-0 psf-price-on-request">
                                                        {{ translate('Contact_us_for_the_price') }}
                                                    </h3>
                                                @endif

                                                {{-- PSF: availability --}}
                                                @if (psfIsOnOrder($product))
                                                    <span class="badge badge-soft-warning mt-2 psf-availability">
                                                        {{ translate('On_Order') }}
                                                    </span>
                                                @endif
                                            </div>
                                            ...
                                            <div class="">
                                                <div class="product-quantity d-flex flex-column __gap-15">
                                                    ...
                                                    {{-- PSF: hide running total when the product has no price --}}
                                                    @if (psfHasPrice($product))
                                            ...
                                            <div class="__btn-grp product-add-and-buy-section-parent">
                                                ...
                                                {{-- PSF: no price -> ask on WhatsApp instead of cart --}}
                                                @if (!psfHasPrice($product))
                                                    <div class="d-flex gap-2 psf-price-request-section">
                                                        <a href="{{ psfPriceRequestUrl($product) }}"
                                                           target="_blank" rel="noopener"
                                                           class="btn btn--primary psf-accent-btn element-center gap-2 psf-request-price-btn">
                                                            <i class="fa fa-whatsapp"></i>
                                                            <span class="string-limit">{{ translate('Request_the_price') }}</span>
                                                        </a>
                                                    </div>
                                                @else
                                                <div
                                                    ...
                                                    @else
                                                        ...
                                                        {{-- PSF: brief §8 — a priced product still gets a WhatsApp button.
                                                             Sits in this same flex row so it lines up with Buy now /
                                                             Add to cart; gap-2 spaces the icon from the label
                                                             (the theme has no me-* utilities). --}}
                                                        @php($psfWa = psfWhatsappOrderData($product))
                                                        <a href="{{ psfOrderRequestUrl($product) }}"
                                                           target="_blank" rel="noopener"
                                                           data-psf-wa-number="{{ $psfWa['number'] }}"
                                                           data-psf-wa-template="{{ $psfWa['template'] }}"
                                                           class="btn bg-whatsapp text-white element-center gap-2 psf-order-whatsapp-btn">
                                                            <i class="fa fa-whatsapp"></i>
                                                            <span class="string-limit">{{ translate('Order_on_WhatsApp') }}</span>
                                                        </a>
                                                    @endif
                                                </div>
                                                @endif {{-- PSF: end price / no-price split --}}
...
@push('script')
    ...
    <script>
        ...
        /* PSF (client brief §8): build the WhatsApp order message at click time,
           so it carries the quantity and the variation the visitor actually
           picked. The wording itself comes from Paramètres PSF — nothing here
           is fixed text. */
        $(document).on('click', '.psf-order-whatsapp-btn', function (e) {
            var $btn = $(this);
            var number = $btn.data('psf-wa-number');
            var template = $btn.data('psf-wa-template');

            if (!number || !template) {
                return; // fall through to the plain href
            }

            var $form = $('.add-to-cart-details-form');

            var quantity = $.trim($form.find('input[name="quantity"]').val() || '') || '1';

            // "Diamètre : 25 mm" for every attribute the visitor selected
            var parts = [];
            $form.find('input[type="radio"]:checked').each(function () {
                var $radio = $(this);
                var label = $.trim($radio.closest('.row').find('.product-description-label').first().text());
                var value = $.trim($radio.val() || '');
                if (value !== '') {
            ...
            // drop any line left empty once the placeholders are filled
